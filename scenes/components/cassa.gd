...
@export var oggetti: Array[String] = []
...
func apri() -> void:
	...
	for nome in oggetti:
		Inventario.aggiungi_oggetto(nome)
...
func _testo_bottino() -> String:
	...
	for nome in conteggio:
		var n: int = conteggio[nome]
		parti.append(nome if n == 1 else "%s x%d" % [nome, n])
	if parti.is_empty():
